...
class Solution:
	def root_to_leaf_path_sum(self, node, rem_diff):
		if node is None:
			return False
		...
		if node.left is not None:
			if self.root_to_leaf_path_sum(node.left, abs(node.data - rem_diff)):
				return True
		if node.right is not None:
			if self.root_to_leaf_path_sum(node.right, abs(node.data - rem_diff)):
				return True
		return False
